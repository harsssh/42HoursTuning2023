...
const exeQuery = async (
  ...
  let sqlQuery =  `SELECT user_id, user_name, office_id, user_icon_id FROM ( SELECT user.*, department_name FROM user   
...
export const getAllCandidates = async (
  ...
  while (members.length < matchGroupConfig.numOfMembers) {
    const offset = Math.floor(Math.random() * (300000 - 2000));
